...
public class AStarAlgorithm {
	...
	// all nodes/states are on the grid
	...
	public void search() {
		...
		// the algorithm terminates when there is no item left in open set
		while(!openSet.isEmpty()) {
			//poll: returns the node with smallest f=g+h value
			...
			// visit all the neighbors of actual node
			for(Node neighbor : getAllNeighbors(currentNode)) {
				
				// we have already considered the state so go on
